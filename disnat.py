...
data.sort_values(
    by=["Date de règlement", "Date de transaction"],
    ascending=[True, True],
    inplace=True,
)
...
for index, row in data.iterrows():
    ...
    data["Commission payée"] = addSECFeesToCommission(
        row["Quantité"],
        row["Prix"],
        row["Montant de l''opération"],
        row["Commission payée"],
        secType,
        row["Type de transaction"],
    )
    if secType == "Actions":
        if "ACHAT" in row["Type de transaction"]:
            ...
            "TRSF" in row["Type de transaction"]
            and row["Date de règlement"] == successionTransferDate
            and (row["Prix"] != "" and row["Prix"] != None)
        ):
            secu = getStock(row["Symbole"], row["Description"], row["Devise du prix"])
            ...
            secu.buy(
                ...
                makeNum(row["Commission payée"]),
                date,
                row["Description"] + "Transfert de succession",
            )
        elif "FAILLI" in row["Type de transaction"]:
            date = row["Date de règlement"]
            try:
                secu = getStock(
                    row["Symbole"], row["Description"], row["Devise du prix"]
                )
                date = row["Date de règlement"]
                sellPrice = 0
                commission = 0
                ...
                print(
                    ...
                    + " le "
                    + str(date)
                    + ": Veuillez entrer entrer le symbole de l'entreprise qui a fait faillite\n"
                )
            ...
            try:
                ...
                print(
                    ...
                    + " le "
                    + str(date)
                    + ": Veuillez entrer le vieux symbole d'entreprise dans la rangée symbole et le nouveau symbole dans la rangée description\n"
                )

    elif secType == "Options":
        if "ACHAT" in row["Type de transaction"]:
            secu = getOption(
                row["Symbole"], row["Description"], row["Devise du prix"], date
            )
            secu.buy(
                int(row["Quantité"]),
                makeNum(row["Prix"]),
