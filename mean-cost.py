...
def removeUnusedCol(df: pd.DataFrame) -> pd.DataFrame:
    df.drop(
        ["Account #", "Activity Type", "Account Type", "Settlement Date"],
        axis=1,
        inplace=True,
    )
    return df
...
data.sort_values(
    by=["Symbol", "Transaction Date"], ascending=[True, True], inplace=True
)
...
for index, row in data.iterrows():
    ...
    if (
        row["Action"] == "Buy" and tot < 0
    ):  # we shored and are in the process of closing the short
        if type == "Option":
    ...
    if row["Action"] == "REV":  # reverse split
        try:
            ...
            data.at[index, "Other Symbols"] += "".join(
                (otherSymbol) for otherSymbol in otherSymbols
            )
            print("Calculated")
        except:
    ...
    if row["Action"] == "ASN":
        ...
        if option == "PUT":
            subString = re.findall(
                "[0-9]{2}/[0-9]{2}/[0-9]{2} [0-9]+", row["Description"]
            )[0]
