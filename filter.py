...
def _removeUnsupportedData(df: pd.DataFrame) -> pd.DataFrame:
    ...
    )  # Electronic fund transfer. Not a taxable event
    ...
    df = df.drop(
        df[df["Action"] == "FCH"].index
    )  # Only on 2020 (Data fees, rebates etc)
    df = df.drop(df[df["Action"] == "TF6"].index)  # Only happened in TFSA
    df = df.drop(
        df[df["Action"] == "REV"].index
    )  # 2020 SQQQ reverse split (position closed in 2020)
    df = df.drop(
        df[df["Activity Type"] == "Interest"].index
    )  # only in 2020, not a transaction
    df = df.drop(
        df[df["Activity Type"] == "Dividends"].index
