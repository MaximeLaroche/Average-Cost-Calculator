...
class Stock:
    ...
    def _adjTickerName(self, ticker: str) -> str:
        ...
        if ticker.startswith("."):
            ...
            ticker = ticker.split("XTSE:", 1)[
                1
            ]  # Disnat prix de succession entré avec excel
            ticker += ".TO"
    ...
    def buy(
        ...
        if quantity != 0:  # the buy to closde may have put the remaining to 0
            ...
            self.avg = (self.total * self.avg + price * quantity) / (
                self.total + quantity
            )
            self.total += quantity
            data = {
    ...
    def _buyToClose(
        self, quantity: number, price: number, date: datetime, description: str
    ):
        self.description = description
    ...
    def _shortSell(
        ...
        self._updateRate(quantity, price, date)
        self.avg = (self.avg * self.total + quantity * price) / (
            abs(self.total) + quantity
        )
        self.description = description
        self.total -= quantity
        data = {
    ...
    def _add(self, obj: Dict):
        ...
        df = self.getDf()
        df = pd.concat([df, pd.DataFrame.from_records([obj])])
        self.setDf(df)
    ...
    def _sort(df: pd.DataFrame) -> pd.DataFrame:
        df.sort_values(
            by=[LABELS.id, LABELS.date, LABELS.index],
    ...
    def _updateRate(self, quantity: number, price: number, date: datetime):
        ...
        if (self.avg * total + quantity * price) != 0:
            self.avgRate = (
                self.avgRate * self.avg * total + rate * quantity * price
            ) / (self.avg * total + quantity * price)
